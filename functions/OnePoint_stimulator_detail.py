...
class One_point:
    ...
    def OnePoint_TI_wave(location, amp1_1, amp1_2, freq1, freq2, delay, dur, end, imp_am, imp_location, sp_rate):
        ...
        def TI_result(location, delay, duration, sampling_rate, end_time, stim_t, plus_wave, impulse):
            ...
            #fig_r.add_trace(go.Scatter(x=temp2, y=impulse, name="AP_generator"))
            #fig_r.add_trace(go.Scatter(x=stim_t, y=stim_vector2, name="injected wave2"))
            #fig_r.add_trace(go.Scatter(x=t, y=axon0_v_9, name="axon(0_9)_v", line=dict(color="#3B1877")))
            #fig_r.add_trace(go.Scatter(x=t, y=axon5_v, name="axon(5_8)_v", line=dict(color="#DA5A2A")))
            fig_r.update_layout(
                width=800,
